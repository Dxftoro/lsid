...
namespace lsid {
	...
	namespace literals {
		...
		uint32_t operator""_seq(const char* input, size_t size) {
			return lsid::constSequential<uint32_t>(input);
		}

		uint32_t operator""_seq32(const char* input, size_t size) {
			return lsid::constSequential<uint32_t>(input);
		}

		uint64_t operator""_seq64(const char* input, size_t size) {
